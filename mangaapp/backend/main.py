...
# In-memory page cache to avoid re-extracting
_page_cache: dict = {}
...
def scan_manga_dir(force=False):
    ...
    import time
    now = time.time()
    # Only rescan if more than 30 seconds since last scan
    ...
    try:
        ...
        for item in MANGA_DIR.iterdir():
            if item.is_dir():
                ...
                if not existing:
                    chapters = sorted([f for f in item.iterdir() if f.suffix.lower() in SUPPORTED_FORMATS])
                    cover = None
                    if chapters:
                        try:
...
# ── Cache static serve with proper headers ────────────────────────────────────
