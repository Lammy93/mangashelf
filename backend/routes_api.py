...
from .db import db_conn, get_scan_setting, set_scan_setting, get_all_manga_directories, slugify
from .models import (
...
from .services import (
    ...
    auto_fetch_metadata, pre_extract_pages,
    _run_download_job, search_mangal_all, trigger_interval_reset
)
from .job_queue import create_job, get_job, list_jobs, cancel_job, retry_job, retry_all_failed, clear_completed
...
def _get_pdf_doc(file_path: Path):
    with _pdf_doc_cache_lock:
        key = str(file_path)
        if key in _pdf_doc_cache:
            doc = _pdf_doc_cache[key]
            if doc.is_open:
                return doc
